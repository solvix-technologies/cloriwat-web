...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => acuasafe_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => acuasafe_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => acuasafe_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => acuasafe_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => acuasafe_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => acuasafe_meta( 'banner_elementor_template', $page_id ),
				'banner'            => acuasafe_set( acuasafe_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => acuasafe_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => acuasafe_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : acuasafe_the_title( $template ),
				'banner'        => acuasafe_set( $bg, 'url' ),
