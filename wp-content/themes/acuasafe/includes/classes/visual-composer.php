...
class Visual_Composer {
	...
	function init() {
		...
		$maps = apply_filters( 'acuasafe_vc_map', $this->maps );
		...
		foreach ( $maps as $value) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( function_exists( 'wpacuasafe_shortcode' ) ) {
					$tag = esc_attr( acuasafe_set( $data, 'base' ) );

					wpacuasafe_shortcode( $tag, array( $this, 'output' ) );
				}
			}
		}
	}
	...
	function output( $atts, $content = null, $tag ) {
		...
		acuasafe_template_load( 'shortcodes/' . $tag . '.php', compact( 'atts', 'content', 'tag' ) );
	...
	function get_file( $tag ) {
		...
		$file = acuasafe_template( 'includes/resource/vc_map/'.$tag . '.php' );
		$file = apply_filters( "acuasafe_vc_map_file_{$tag}", $file );
	...
	function shortcodeParams( $tag ) {
		...
		if ( file_exists( $file ) ) {
			...
			return acuasafe_set( $data, 'params' );
		}
	...
	function toggle( $settings, $value ) {
		...
		$values = isset( $settings['value'] ) && is_array( $settings['value'] ) ? $settings['value'] : array( esc_html__( 'Yes', 'acuasafe' ) => 'true' );
		if ( ! empty( $values ) ) {
