...
function acuasafe_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('acuasafe_redux_custom_fonts_load', 'acuasafe_redux_custom_fonts_load');
function acuasafe_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = acuasafe_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/acuasafe/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [acuasafe_social_share_output description]
 *
 ...
 */
function acuasafe_social_share_output( $icon, $color = false ) {
	...
	if ( $icon == 'facebook' ) {
		...
		<li>
			<a onClick="window.open('http://www.facebook.com/sharer.php?u=<?php echo esc_url( $permalink ); ?>', 'Facebook', 'width=600,height=300,left=' + (screen.availWidth / 2 - 300) + ',top=' + (screen.availHeight / 2 - 150) + '');
				...
				<i class="fab fa-facebook-f" <?php echo wp_kses( $fb, $allowed_html ); ?>></i>
			</a>
		</li>
	<?php } ?>
	<?php
	...
	<?php if ( $icon == 'pinterest' ) {
		...
		<li>
			<a href='javascript:void((function(){var e=document.createElement(&apos;script&apos;);e.setAttribute(&apos;type&apos;,&apos;text/javascript&apos;);e.setAttribute(&apos;charset&apos;,&apos;UTF-8&apos;);e.setAttribute(&apos;src&apos;,&apos;http://assets.pinterest.com/js/pinmarklet.js?r=&apos;+Math.random()*99999999);document.body.appendChild(e)})())'>
				<i class="fab fa-pinterest" <?php echo wp_kses( $pinterest, $allowed_html ); ?>></i></a>
		</li>
	<?php } ?>
	<?php
	if ( $icon == 'stumbleupon' ) {
...
function acuasafe_get_social_icons()
{
	...
	if( acuasafe_set( $options, 'social_media' ) && is_array( acuasafe_set( $options, 'social_media' ) ) )
	{
		$total = count( acuasafe_set( $options, 'social_media' ) ) - 2;
		foreach( acuasafe_set( $options, 'social_media' ) as $social_icon ){
			if( isset( $social_icon['tocopy' ] ) ) continue;
			$title = acuasafe_set( $social_icon, 'title');
			$class = acuasafe_set( $social_icon, 'class');
			$link = acuasafe_set( $social_icon, 'social_link');
			$icon = acuasafe_set( $social_icon, 'social_icon');
...
add_action('acuasafe_social_share_output', 'acuasafe_social_share_output');
...
add_action('acuasafe_social_share_output_two', 'acuasafe_social_share_output_two');
